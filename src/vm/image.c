...
struct object *read_object(FILE *img, int obj_num, struct object *obj)
{
    int size;
    uint32_t offset;
...
int fileIn_version_3(FILE *img)
{
    ...
    while((intptr_t)current_obj < (intptr_t)memoryTop) {
        ...
        current_obj = read_object(img, object_count, current_obj);
        object_count++;
    }
...
uint32_t addr_to_offset(struct object *oop)
{
    ...
    ptrdiff_t offset = top_int - oop_int;  /* in bytes! */
...
struct object *offset_to_addr(uint32_t raw_offset)
{
    ...
    intptr_t top_int = (intptr_t)memoryTop;
    intptr_t oop_int = top_int - offset;
