...
int symbolBareCmp(const char * left, uint32_t leftsize, const char * right, uint32_t rightsize)
{
...
struct object *newSymbol(char * text)
{
    ...
    for (i = 0; i < symbolTop; i++) {
        if (symbolBareCmp(text, strlen(text), (char *)bytePtr(oldSymbols[i]), SIZE(oldSymbols[i])) == 0) {
            return oldSymbols[i];
        }
    }
